...
public:
  ...
  virtual const void LoadImageInWindow(const std::string name, cv::Mat &mat);
	...
	virtual const cv::Mat Contrast(const cv::Mat mat, const double contrastPercentage);
	virtual const cv::Mat Brighten(const cv::Mat mat, const int brightnessOffset);
